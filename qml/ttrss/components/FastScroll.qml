    ...
    Item {
        ...
        Image {
            ...
            Text {
                ...
                x: 14
                y: (listView.section.criteria == ViewSection.FullString) ? 55 : 40

                font.pixelSize: (listView.section.criteria == ViewSection.FullString)
                                ? platformStyle.fullStringFontPixelSize
                                : platformStyle.firstCharacterFontPixelSize
    ...
    QtObject {
        ...
        function adjustContentPosition(y) {
            ...
            if (internal.curSect != sect) {
                ...
                // FIXME - This hack looks nice, but it only works with list elements of equal height.
                // Also, it's very slow if many delegates have to be generated.
                /*
                scroll.complete()
                scroll.from = listView.contentY
                listView.positionViewAtIndex(idx, ListView.Beginning);
                scroll.to = listView.contentY
                listView.contentY = scroll.from
                scroll.start()
                */
            }
        }
